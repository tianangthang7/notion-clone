...
export const files = pgTable("files", {
	...
	workspace_id: uuid("workspace_id").references(() => folders.id, { onDelete: "cascade" }).references(() => workspaces.id, { onDelete: "cascade" }),
});
...
export const users = pgTable("users", {
	...
	email: text("email"),
	updated_at: timestamp("updated_at", { withTimezone: true, mode: 'string' }).default(sql`now()`).notNull(),
},
	(table) => {
		return {
